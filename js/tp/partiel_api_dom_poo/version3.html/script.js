onload = function (){
    ...
    class Releve {
        ...
        fold (tr){
            let mois = tr.getElementsByTagName('td')[0].textContent;
            this.temp.forEach( element => {
               if (element.mois == mois){
                    element.tp.forEach(element => {
        ...
        unfold (tr) {
            let tds = tr.getElementsByTagName('td');
            for ( let i = tds.length; i > 4; i--){
                tr.removeChild(tds[i-1])
            }
        }
        ...
        display (){
            ...
            let my_trs = document.getElementsByTagName('tbody')[0].getElementsByTagName("tr");
            for ( let i = 0; i < my_trs.length; i++){
                my_trs[i].onmouseover = () => {
                    this.color_green(my_trs[i]);
                }
                my_trs[i].onmouseout = () => {
                    this.color_grey(my_trs[i]);
                }

                my_trs[i].onclick = () => {
                    if (my_trs[i].getElementsByTagName('td').length == 4)
                        this.fold(my_trs[i]);
                    else
                        this.unfold(my_trs[i]);
    ...
    let releve = new Releve(temp, body);
    releve.display();
