...
class masspricefree extends Module
{
    ...
    private function _postProcess()
    {
        if (Tools::getValue('masspricefree_wtd', 1) == 1) {
            if (Tools::getValue('masspricefree_type')==2)
            {
                Db::getInstance(_PS_USE_SQL_SLAVE_)->execute('UPDATE `' . _DB_PREFIX_ . 'product_attribute_shop` SET price=price+price*' . Tools::getValue('masspricefree_value', 0) . '/100 WHERE id_shop="' . Tools::getValue('masspricefree_id_shop') . '"');
            }
            Db::getInstance(_PS_USE_SQL_SLAVE_)->execute('UPDATE `' . _DB_PREFIX_ . 'product_shop` SET price=price+price*' . Tools::getValue('masspricefree_value', 0) . '/100 WHERE id_shop="' . Tools::getValue('masspricefree_id_shop') . '"');
        } else {
            if (Tools::getValue('masspricefree_type')==2)
            {
                Db::getInstance(_PS_USE_SQL_SLAVE_)->execute('UPDATE `' . _DB_PREFIX_ . 'product_attribute_shop` SET price=price-price*' . Tools::getValue('masspricefree_value', 0) . '/100  WHERE id_shop="' . Tools::getValue('masspricefree_id_shop') . '"');
